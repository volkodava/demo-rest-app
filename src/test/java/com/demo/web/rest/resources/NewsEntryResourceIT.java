...
public class NewsEntryResourceIT {
    ...
    public void testAuthenticationWithValidCredentials() throws Exception {
        final String URL = endpointUrl + "/rest/news";

        logger.info("Service url: {}", URL);

        UserTransfer adminCredential = loginAsAdmin();
        String token = adminCredential.getToken();
        ...
        ClientResponse response = webResource.header("X-Auth-Token", token).
        ...
        List<NewsEntry> entities = response.getEntity(new GenericType<List<NewsEntry>>() {
        });
    ...
    private UserTransfer loginAsAdmin() {
